...
/**
 * Merge two sorted arrays - FIXED VERSION
 */
DynamicArray* merge_sorted_arrays(DynamicArray* arr1, DynamicArray* arr2) {
    ...
    // Handle NULL arrays by creating empty ones
    DynamicArray empty1 = {NULL, 0, 0};
    DynamicArray empty2 = {NULL, 0, 0};
    
    DynamicArray* a1 = arr1 ? arr1 : &empty1;
    DynamicArray* a2 = arr2 ? arr2 : &empty2;
    
    int i = 0, j = 0;
    
    while (i < a1->size && j < a2->size) {
        if (a1->data[i] <= a2->data[j]) {
            insert_at_end(result, a1->data[i]);
            i++;
        } else {
            insert_at_end(result, a2->data[j]);
            j++;
        }
    }
...
int main() {
    ...
    printf("Update position 1 to 99: %s\n", update_at_position(arr, 1, 99) ? "Success" : "Failed");
    print_array(arr);
